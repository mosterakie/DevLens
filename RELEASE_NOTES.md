...
- 数据库迁移在 api 与 worker 启动时自动执行，用 advisory lock 串行化，
  并记录校验和以发现历史迁移被改动

## 验证情况

145 个测试，其中 31 个针对真实 PostgreSQL（事务回滚、幂等、迁移）。
CI 在每次 push 时执行格式检查、vet、竞态检测、集成测试和镜像构建。
...
有几处看起来像缺陷的地方其实是刻意的选择，写在这里以免被误读。

**分类是模型的自由文本，格式不统一。** 我们没有做归一化，界面上也因此
称它为"标签"而不是"分类"——后者会让人以为背后有受控词表。代价是它不
适合当过滤条件，收益是不必维护一份枚举，模型能对新出现的故障类型给
出自己的归纳。

**AI 会判断出错。** 这一点无法消除，只能控制它的影响：产品上不给单一
结论，而是给候选原因列表加置信度，并保留可核对的证据让用户自己判断。
置信度会随信息量波动——证据充分时给 0.8 以上，日志信息不足时会降到
0.4 并说明缺什么，而不是硬凑一个答案。
