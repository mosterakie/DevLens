...
模型输出可能撞上 `max_tokens` 上限被截断，此时内容是一段断裂的 JSON。
这属于确定性失败：重试会得到同样被截断的结果，只会白费两次调用。

所以实现里单独识别服务端的 `finish_reason == "length"`，遇到就立即
返回错误并提示调高 `LLM_MAX_TOKENS`，而不是等 JSON 解析失败再重试。

实测 `deepseek-flash` 分析一条普通日志约消耗 2400 tokens，早期设的
2048 会把 `suggested_actions` 截在半截字符串中间。默认值已改为 4096，
并在系统提示词里要求 summary 最多两句、各列表最多四项，从源头压缩输出。
